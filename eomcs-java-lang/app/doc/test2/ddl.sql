...
-- 강의강사
DROP TABLE IF EXISTS ems_teacer_lecture RESTRICT;
...
-- 강의강사
CREATE TABLE ems_teacer_lecture (
  lno INTEGER NOT NULL COMMENT '강의번호', -- 강의번호
  mno INTEGER NOT NULL COMMENT '회원번호' -- 회원번호
)
COMMENT '강의강사';
...
-- 강의강사
ALTER TABLE ems_teacer_lecture
  ADD CONSTRAINT PK_ems_teacer_lecture -- 강의강사 기본키
    PRIMARY KEY (
      lno, -- 강의번호
      mno  -- 회원번호
    );
...
CREATE TABLE ems_room (
  rno   INTEGER     NOT NULL COMMENT '강의실 번호', -- 강의실 번호
...
ALTER TABLE ems_room
  MODIFY COLUMN rno INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의실 번호';
...
CREATE TABLE ems_school (
  ...
  titile VARCHAR(50) NOT NULL COMMENT '학교명' -- 학교명
)
COMMENT '학교';
...
CREATE TABLE ems_department (
  dno    INTEGER     NOT NULL COMMENT '부서번호', -- 부서번호
  titile VARCHAR(50) NOT NULL COMMENT '부서명' -- 부서명
)
COMMENT '부서';
...
CREATE TABLE ems_center (
  cno      INTEGER      NOT NULL COMMENT '교육센터번호', -- 교육센터번호
  titile   VARCHAR(50)  NOT NULL COMMENT '센터명', -- 센터명
...
-- 강의강사
ALTER TABLE ems_teacer_lecture
  ADD CONSTRAINT FK_ems_lecture_TO_ems_teacer_lecture -- 강의 -> 강의강사
    FOREIGN KEY (
      lno -- 강의번호
    )
    REFERENCES ems_lecture ( -- 강의
...
-- 강의강사
ALTER TABLE ems_teacer_lecture
  ADD CONSTRAINT FK_ems_teacher_TO_ems_teacer_lecture -- 강사 -> 강의강사
    FOREIGN KEY (
      mno -- 회원번호
    )
