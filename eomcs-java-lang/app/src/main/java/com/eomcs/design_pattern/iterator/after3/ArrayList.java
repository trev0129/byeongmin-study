...
public class ArrayList<E> {
  ...
  public Iterator<E> iterator() {
    // 생성자를 호출할 때 바깥 클래스의 인스턴스 주소를 넘겨 주지 않아도
    // 컴파일러가 넘겨주는 코드로 자동변환한다.
    //
  ...
  class ArrayListIterator<T> implements Iterator<T> {
    ...
    // 논 스태틱인 경우 바깥 클래스의 인스턴스 주소를 받을 필드가 자동 추가된다.
    // 따라서 다음과 같이 개발자가 직접 추가할 필요가 없다.

    // ArrayList<E> list; // static을 빼면 자동생성 된다.
    ...
    // 개발자가 직접 추가할 필요가 없다.
    //
    //    public ArrayListIterator(ArrayList<E> list) { // static을 빼면 자동 생성된다.
    //      this.list = list;
    //    }
    ...
    public T next() {
      ...
      // ArrayList.get()의 리턴타입이 E이다.
      // E가 가리키는 타입 정보는 다음과 같이 결국 ArrayListIterator를 만들 때 넘겨진다.
      //    new ArrayListIterator<E>();
      // 따라서 ArrayListIterator 내부에서 사용하는 T가 곧 ArrayList의 E 와 같다.
